...
#-------------------------------New Model for  Reconstruction--------------------------------------------------
import torch
import torch.nn as nn
import torch.nn.functional as F

class InputProj3D(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size=3, stride=1, act_layer=nn.LeakyReLU):
        super(InputProj3D, self).__init__()
        self.proj = nn.Sequential(
            nn.Conv3d(in_channel, out_channel, kernel_size=kernel_size, stride=stride, padding=1),
            act_layer(inplace=True)
        )
...
class OutputProj3D(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size=3, stride=1):
        super(OutputProj3D, self).__init__()
        self.proj = nn.Conv3d(in_channel, out_channel, kernel_size=kernel_size, stride=stride, padding=1)

    def forward(self, x):
        return self.proj(x)

class BasicUformerLayer3D(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 norm_layer=nn.LayerNorm, use_checkpoint=False, token_projection='linear', token_mlp='leff',
                 shift_flag=True, modulator=False, cross_modulator=False):
        super(BasicUformerLayer3D, self).__init__()
        self.dim = dim
    ...
    def forward(self, x, mask=None):
        B, C, D, H, W = x.shape
        pad_d = (self.win_size - D % self.win_size) % self.win_size
        ...
        print(f"BasicUformerLayer3D forward, input shape: {x.shape}")
        return x

def window_partition3D(x, win_size):
    B, C, D, H, W = x.shape
    x = x.view(B, C, D // win_size, win_size, H // win_size, win_size, W // win_size, win_size)
    return x.permute(0, 2, 4, 6, 1, 3, 5, 7).contiguous().view(-1, win_size, win_size, win_size, C)

class Downsample3D(nn.Module):
    def __init__(self, in_channel, out_channel):
        super(Downsample3D, self).__init__()
        self.down = nn.Conv3d(in_channel, out_channel, kernel_size=3, stride=2, padding=1)
...
class Upsample3D(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.up = nn.ConvTranspose3d(in_channel, out_channel, kernel_size=3, stride=2, padding=1, output_padding=1)

    def forward(self, x):
        return self.up(x)

class ImageRestorationUformer3D(nn.Module):
    def __init__(self, img_size=(27, 600, 600), embed_dim=32, enc_depths=[2, 2, 2, 2], dec_depths=[2, 2, 2, 2],
                 enc_num_heads=[1, 2, 4, 8], dec_num_heads=[8, 4, 2, 1],
                 win_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, token_projection='linear', token_mlp='leff',
                 dowsample=Downsample3D, upsample=Upsample3D, shift_flag=True, modulator=False,
                 cross_modulator=False, **kwargs):
        super().__init__()
        ...
        assert img_size[1] % win_size == 0 and img_size[2] % win_size == 0, "Input dimensions must be divisible by the window size"

        enc_dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(enc_depths))]
        dec_dpr = enc_dpr[::-1]

        self.input_proj = InputProj3D(in_channel=img_size[0], out_channel=embed_dim, kernel_size=3, stride=1,
                                      act_layer=nn.LeakyReLU)
        self.output_proj = OutputProj3D(in_channel=embed_dim, out_channel=1, kernel_size=3, stride=1)

        self.encoderlayers = nn.ModuleList()
        self.dowsamples = nn.ModuleList()
        for i in range(self.num_enc_layers):
            layer = BasicUformerLayer3D(dim=embed_dim * (2 ** i),
                                        output_dim=embed_dim * (2 ** i),
                                        input_resolution=(img_size[1] // (2 ** i), img_size[2] // (2 ** i)),
                                        depth=enc_depths[i],
                                        ...
                                        qkv_bias=qkv_bias, qk_scale=qk_scale,
                                        drop=drop_rate, attn_drop=attn_drop_rate,
                                        drop_path=enc_dpr[sum(enc_depths[:i]):sum(enc_depths[:i + 1])],
                                        norm_layer=norm_layer,
                                        use_checkpoint=use_checkpoint,
                                        token_projection=token_projection, token_mlp=token_mlp,
                                        shift_flag=shift_flag)
            ...
            if i < self.num_enc_layers - 1:
                self.dowsamples.append(Downsample3D(embed_dim * (2 ** i), embed_dim * (2 ** (i + 1))))

        self.upsamples = nn.ModuleList()
        self.decoderlayers = nn.ModuleList()
        self.conv_after_concat = nn.ModuleList()
        for i in range(self.num_dec_layers):
            layer = BasicUformerLayer3D(dim=embed_dim * (2 ** (self.num_dec_layers - i - 1)),
                                        output_dim=embed_dim * (2 ** (self.num_dec_layers - i - 1)),
                                        input_resolution=(img_size[1] // (2 ** (self.num_dec_layers - i - 1)),
                                                          img_size[2] // (2 ** (self.num_dec_layers - i - 1))),
                                        depth=dec_depths[i],
            ...
            self.conv_after_concat.append(nn.Conv3d(embed_dim * (2 ** (self.num_dec_layers - i - 1)) * 2,
                                                    embed_dim * (2 ** (self.num_dec_layers - i - 1)), kernel_size=1))
            if i < self.num_dec_layers - 1:
                self.upsamples.append(Upsample3D(embed_dim * (2 ** (self.num_dec_layers - i - 1)),
                                                 embed_dim * (2 ** (self.num_dec_layers - i - 2))))
    ...
    def forward(self, x, mask=None):
        ...
        y = self.input_proj(x)
        y = self.pos_drop(y)
        print(f"After input_proj shape: {y.shape}")
        convs = []
        for i in range(self.num_enc_layers):
            y = self.encoderlayers[i](y, mask=mask)
            print(f"Encoder layer {i} output shape: {y.shape}")
            convs.append(y)
            if i < self.num_enc_layers - 1:
                y = self.dowsamples[i](y)
                print(f"After downsample {i} shape: {y.shape}")

        for i in range(self.num_dec_layers):
            if i < self.num_dec_layers - 1:
                y = self.upsamples[i](y)
                print(f"After upsample {i} shape: {y.shape}")
            skip = convs[self.num_enc_layers - i - 1]
            if y.shape[2:] != skip.shape[2:]:
                pad_d = (skip.shape[2] - y.shape[2]) // 2
                pad_h = (skip.shape[3] - y.shape[3]) // 2
                pad_w = (skip.shape[4] - y.shape[4]) // 2
                y = F.pad(y, (pad_w, pad_w, pad_h, pad_h, pad_d, pad_d), "constant", 0)
            y = torch.cat([y, skip], dim=1)
            y = self.conv_after_concat[i](y)  # 调整通道数
            print(f"After concat and conv {i} shape: {y.shape}")
            y = self.decoderlayers[i](y, mask=mask)
            print(f"Decoder layer {i} output shape: {y.shape}")
...
if __name__ == "__main__":
    input_size = (27, 600, 600)
    enc_depths = [2, 2, 2, 2]
    dec_depths = [2, 2, 2, 2]
    enc_num_heads = [1, 2, 4, 8]
    dec_num_heads = [8, 4, 2, 1]
    model_restoration = ImageRestorationUformer3D(img_size=input_size, embed_dim=16, enc_depths=enc_depths, dec_depths=dec_depths,
                                                  ...
                                                  shift_flag=False)
    x = torch.randn(1, 27, 600, 600)
    x = x.unsqueeze(2)  # 添加深度维度
    y = model_restoration(x)
    print(y.shape)  # 输出应为 (1, 1, 27, 600, 600)
